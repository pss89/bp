            ...
            <ul className="flex flex-col font-medium p-4 md:p-0 mt-4 border rounded-lg 
              ...
              {navbarItems.map((item, index) => {
                if (item.subItems) {
                  return (
                    <li key={index}>
                      <button id="dropdownNavbarLink" className="flex items-center justify-between w-full py-2 px-3 rounded-sm 
                        md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 md:w-auto text-white 
                        md:dark:hover:text-blue-500 dark:focus:text-white border-gray-700 hover:bg-gray-700 
                        md:dark:hover:bg-transparent" onClick={dropDownMenuOpen({index})}>{item.title} 
                          ...
                          {item.subItems.map((subItem, key) => (
                            <li key={key}>
                              <Link key={key} to={subItem.path} className="block px-4 py-2 hover:bg-gray-600 
                                dark:hover:text-white">{subItem.title}</Link>
                            </li>
                          ))}
                        </ul>
                      </div>
                  ...
                  return (
                    <li key={index}>
                      <Link to={item.path} className="block py-2 px-3 ounded-sm md:hover:bg-transparent md:border-0 
                        md:p-0 text-white md:hover:text-blue-500 hover:bg-gray-700 
                        hover:text-white md:dark:hover:bg-transparent">{item.title}</Link>
                    </li>
                  );
                }
              })}
